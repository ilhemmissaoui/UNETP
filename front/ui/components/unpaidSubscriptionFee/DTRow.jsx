            ...
            <>
                ...
                <span className="d-block">
                    ...
                    <span className="fw-bolder text-primary">
                        ...
                        <FormatPrice
                            value={
                                (capacityHistory?.supContractStudentsCount || 0) *
                                (subscriptionParam?.scSup || 0)
                            }
                        />
                    </span>{' '}
                ...
                <span className="d-block">
                    Total :{' '}
                    <span className="fw-bolder text-primary">
                        <FormatPrice value={subscriptionFees?.calculatedAmount} />
                    </span>{' '}
                </span>
                {!!subscriptionFees?.customAmount && (
                    <span className="d-block">
                        ...
                        <span className="fw-bolder text-primary">
                            <FormatPrice value={subscriptionFees?.customAmount} />
                        </span>{' '}
                    </span>
                )}
            </>
...
const MemeberItem = ({ data }) => {
    ...
    const includedFees = watch('suscriptionFeesIncluded');
    ...
    return (
        <tr>
            <td>
                <div className="form-check form-check-custom form-check-solid ">
                    <input
                        ...
                        type="checkbox"
                        name="suscriptionFees"
                        className={clsx('form-check-input me-3', {
                            'is-invalid': errors?.suscriptionFees
                        })}
                        checked={includedFees?.includes(data.organization?.subscriptionFees[0]?.id)}
                        disabled={disabled}
                        onChange={({ target: { checked } }) => {
                            if (checked) {
                                setValue('suscriptionFeesIncluded', [
                                    ...includedFees,
                                    data.organization?.subscriptionFees[0]?.id
                                ]);
                            } else {
                                setValue(
                                    'suscriptionFeesIncluded',
                                    includedFees?.filter(
                                        (e) => e !== data.organization?.subscriptionFees[0]?.id
                                    )
                                );
                            }
                        }}
            ...
            <td>
                <label htmlFor={data.organization?.establishment?.id}>{organization?.name}</label>
            </td>
            <td>
