...
export const capacityEntryDefaultSchema = yup.object({
    ...
    year: yup
        ...
        .test('isDuplicatedYear', 'Cette année existe déjà', function (value) {
            const { studentCapacityEntries, id } = this.parent;

            const unique = studentCapacityEntries?.find((e) => e.year == value);

            return id ? (unique ? (unique?.id == id ? true : false) : true) : !unique;
        })
});

export const capacityEntrySchema = yup.lazy((v) => {
    switch (v?.keyForm) {
        case '0':
            return yup
                .object({
...
export const multipleCapacityHistory = yup.object({
    capacityHistories: yup.array().of(capacityEntrySchema).min(1)
});
export default capacityEntrySchema;
