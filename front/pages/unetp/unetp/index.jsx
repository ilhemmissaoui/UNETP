...
const Unetp = () => {
    ...
    const fetchOffice = async () => {
        try {
            const { data } = await axios.get(`${endpointUrl}/unetp`);
            reset({
            ...
            console.log(e);
            setToast({
                message: 'Erreur lors de la récupération de UNETP',
                variant: 'danger'
            });
        }
    };
    const submit = async (data) => {
        try {
            await axios.post(`${endpointUrl}/unetp/${boardDirector.id}`, data);
            setToast({
                message: 'UNETP a été mis à jour avec succès',
                variant: 'success'
            });
            await fetchOffice();
        } catch (e) {
            setToast({
                message: 'Erreur lors de mis à jour de UNETP ',
                variant: 'danger'
            });
        }
    };
    ...
    return (
        <Layout>
            <Head>
                <title>Modification du UNETP| {process.env.platformName} </title>
            </Head>
            {isLoading ? (
                    ...
                    <div className="container-fluid">
                        <div className="card">
                            ...
                            <Ability I="save" an="unetp">
                                <div className="card-footer d-flex justify-content-end">
                                    <button className="btn btn-primary" form="edit-unetp">
