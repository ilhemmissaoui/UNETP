...
const DirectorsBoard = () => {
    ...
    const fetchCa = async () => {
        try {
            const { data } = await axios.get(`${endpointUrl}/directors-board`);
            console.log({ data });
            reset({
            ...
            console.log(e);
            setToast({
                message: "Erreur lors de la récupération de Conseil d'administration",
                variant: 'danger'
            });
        }
    };
    ...
    const submit = async (data) => {
        try {
            await axios.post(`${endpointUrl}/directors-board/${boardDirector.id}`, data);
            setToast({
                message: "Conseil d'administration a été mis à jour avec succès",
                variant: 'success'
            });
            await fetchCa();
        } catch (e) {
            setToast({
                message: "Erreur lors de mis à jour de Conseil d'administration ",
                variant: 'danger'
            });
        }
    };
    ...
    return (
        <Layout>
            <Head>
                <title>Modification de conseil administration | {process.env.platformName} </title>
            </Head>
            {isLoading ? (
                    ...
                    <div className="container-fluid">
                        <div className="card">
                            <div className="card-body">
                                ...
                                <FormProviderSafety>
                                    <FormParamsProvider {...componentsParams}>
                                        <form id="edit-ca" onSubmit={handleSubmit(submit)}></form>
                            ...
                            <Ability I="save" an="unetp">
                                <div className="card-footer d-flex justify-content-end">
                                    <button className="btn btn-primary" form="edit-ca">
